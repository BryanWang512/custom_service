...
AndroidAudioTrack::AndroidAudioTrack()
{
	...
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> FindClass");
	...
	//生成AudioTrack
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> NewGlobalRef AudioTrack_j");
	...
	//获取构造函数ID
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> GetMethodID constructor_j");
	...
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> getMinBufferSize getMinBufferSize_j");
	...
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> CallStaticIntMethod getMinBufferSize_j");
	...
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> minBufferSize_j = %d", minBufferSize_j);
	checkJniException(getEnv(), "AndroidAudioTrack::AndroidAudioTrack -> minBufferSize_j = NULL");

	//执行创建AudioTrack
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> createAudioTrack");
	//createAudioTrack(minBufferSize_j, MODE_STREAM);

	//获取getPlayState函数ID
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> GetMethodID getPlayState_j");
	...
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> GetMethodID setStereoVolume_j");
	...
	//获取play函数ID
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> GetMethodID play_j");
	...
	//获取flush函数ID
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> GetMethodID flush_j");
	...
	//获取write函数ID
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> GetMethodID write_j");
	...
	//获取pause函数ID
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> GetMethodID pause_j");
	...
	//获取release函数ID
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> GetMethodID release_j");
	...
	//获取stop函数ID
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::AndroidAudioTrack -> GetMethodID stop_j");
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::~AndroidAudioTrack");
	if (getState() != PLAYSTATE_STOPPED){
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::~AndroidAudioTrack -> getPlayState() != PLAYSTATE_STOPPED");
		stop();
	}
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::~AndroidAudioTrack -> release");
	release();

	if (AudioTrack_j != NULL){
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::~AndroidAudioTrack -> AudioTrack_j != NULL");
...
void AndroidAudioTrack::createAudioTrack(int buffSize, int mode)
{
	...
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::createAudioTrack");
	bufferSize = (buffSize >= minBufferSize_j) ? buffSize : minBufferSize_j;
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::createAudioTrack -> minBufferSize_j = %d, bufferSize = %d", minBufferSize_j, bufferSize);

	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::createAudioTrack -> NewObject audioTrack_j_Local");
	...
	if (audioTrack_j != NULL){
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::createAudioTrack -> audioTrack_j != NULL");
		stop();
		release();
		if (audioTrack_j != NULL){
			kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::createAudioTrack -> getEnv()->DeleteGlobalRef(audioTrack_j)");
			getEnv()->DeleteGlobalRef(audioTrack_j);
			audioTrack_j = NULL;
		}
	}

	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::createAudioTrack -> NewGlobalRef audioTrack_j");
	...
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::createAudioTrack -> DeleteLocalRef audioTrack_j");
	getEnv()->DeleteLocalRef(audioTrack_j_Local);
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::flush");
	if (audioTrack_j == NULL){
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::flush -> audioTrack_j == NULL");
	}
	else{
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::getPlayState");
	int state = -1;
	if (audioTrack_j == NULL){
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::getPlayState -> audioTrack_j == NULL");
	}
	else{
		state = getEnv()->CallIntMethod(audioTrack_j, getPlayState_j);
		checkJniException(getEnv(), "CallIntMethod getPlayState");
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::getPlayState -> playState = %d", state);
	}
	return state;
}
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::pause");
	if (audioTrack_j == NULL){
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::pause -> audioTrack_j == NULL");
	}
	else{
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::play");
	if (audioTrack_j == NULL){
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::play -> audioTrack_j == NULL");
	}
	else{
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::release");
	if (audioTrack_j == NULL){
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::release -> audioTrack_j == NULL");
	}
	else{
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::stop");
	if (audioTrack_j == NULL){
		kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::stop -> audioTrack_j == NULL");
	}
	else{
...
void printByteArray(jbyteArray buffer)
{
	...
	for (int i = 0; i < len; i++){
		kefu_print_log_debug("audio_kefu", "buffer[%d] = %d", i, minut1[i]);
	}
	free(minut1);
}
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::getMinBufferSize -> minBufferSize_j = %d", minBufferSize_j);
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("recorder", "AndroidAudioTrack::setStereoVolume");
	int flag = -1;
	if (audioTrack_j == NULL){
		kefu_print_log_debug("recorder", "AndroidAudioTrack::setStereoVolume -> audioTrack_j == NULL");
	}
	else{
...
{
	lock_guard<recursive_mutex> lock(m_mutex);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::writeHandle");
	createAudioTrack(sizeInBytes, MODE_STATIC);
	setStereoVolume(1.0, 1.0);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::writeHandle -> getEnv()->NewByteArray(sizeInBytes)");
	jbyteArray buffer = getEnv()->NewByteArray(sizeInBytes);
	int len = getEnv()->GetArrayLength(buffer);
	kefu_print_log_debug("audio_kefu", "AndroidAudioTrack::writeHandle -> buffer len = %d", len);

	kefu_print_log_debug("audio_kefu", "getEnv()->SetByteArrayRegion(buffer, 0, sizeInBytes, (jbyte *)audioData)");
	...
	kefu_print_log_debug("audio_kefu", "getEnv()->CallVoidMethod(audioTrack_j, write_j, buffer, 0, sizeInBytes)");
	int flag = getEnv()->CallIntMethod(audioTrack_j, write_j, buffer, 0, sizeInBytes);
	kefu_print_log_debug("audio_kefu", "getEnv()->CallVoidMethod(audioTrack_j, write_j, buffer, 0, sizeInBytes) flag = %d", flag);

	kefu_print_log_debug("audio_kefu", "getEnv()->DeleteLocalRef(buffer)");
