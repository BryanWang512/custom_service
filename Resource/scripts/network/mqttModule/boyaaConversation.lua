require("network/mqttModule/mqttMaps");
require("network/mqttModule/clientConfig");
require("network/mqttModule/dataUtils");
require("network/mqttModule/mqttConstants");
...
BoyaaConversation.setVip = function (self, isVip)
    self.isVip = isVip
end
...
BoyaaConversation.createMqtt = function (self)
    self.mqtt.mqtt_create(self:generateMqttConfig(), self.handlerId);
end
...
function mqtt_message_arrived(handlerId, tag, ...)
    if handlerId == nil or tag == nil then return end
    local args = {...};
    ...
    EventDispatcher.getInstance():dispatch("mqttReceiveProtocal", tag, ...)	
end

--code 是失败代码 
function mqtt_event_callback(handlerId, event, code)

    if handlerId == nil then return end
    local conversation = BoyaaConversation.s_conversations[handlerId];
    ...
    elseif event == MqttEvent_Map.MQTT_DISCONNECT_SUCCESS or event == MqttEvent_Map.MQTT_DISCONNECT_FAILURE then
    	EventDispatcher.getInstance():dispatch("destroyCallback")  
    end
end