require("network/mqttModule/boyaaConversation");
local HTTP2 = require("network.http2")
local UserData = require("conversation/sessionData")

--接收协议后的回调函数
local receviceCallback = require("network/protocalFunc")
...
netWorkControl.init = function ()
	...
	netWorkControl.initClock = Clock.instance():schedule_once(function()
		...
		EventDispatcher.getInstance():register("mqttReceiveProtocal", netWorkControl, netWorkControl.receiveProtocal)
...
netWorkControl.destroy = function (time)
	EventDispatcher.getInstance():unregister("mqttReceiveProtocal", netWorkControl, netWorkControl.receiveProtocal)
	if netWorkControl.destroyClock then
		netWorkControl.destroyClock:cancel()
		netWorkControl.destroyClock = nil
	end
